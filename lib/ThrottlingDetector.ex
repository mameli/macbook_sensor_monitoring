defmodule ThrottlingDetector do
  ...
  def query_sensor_data() do
    ...
    %{count: count, sum: sum, peak: peak} = sd_list |> Enum.reduce(%{sum: 0, count: 0, peak: 0}, fn sd, acc ->
      %{sum: acc.sum + sd.sensor_mean_value, count: acc.count + 1, peak: max(acc.peak, sd.sensor_peak_value)}
    end)
